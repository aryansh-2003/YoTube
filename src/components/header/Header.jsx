...
export default function Header() {
  ...
  return (
    <header className="flex items-center justify-between w-full px-4 sm:px-6 py-2 h-14 fixed top-0 left-0 right-0 z-50 border-b backdrop-blur-lg bg-white/6 border-white/8 shadow-sm" style={{ willChange: 'backdrop-filter' }}>
      ...
      {!mobileSearchOpen && (
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center">
            <img src={chalchitram} className="w-20" alt="Logo" />
          </div>
        </div>
      )}
      ...
      <div className="flex items-center flex-1 max-w-5xl mx-2 sm:mx-8 justify-center">
        ...
        >
          <div className="flex items-center w-full bg-white/6 backdrop-blur-md border border-white/8 rounded-full h-10 px-1">
            <input
              ...
              type="text"
              placeholder="Search"
              className="flex-1 px-4 py-2 bg-transparent focus:outline-none text-white placeholder-gray-300 rounded-full"
            />
            <button type="submit" className="px-4 py-2 rounded-full hover:bg-white/8 transition" aria-label="search">
        ...
        <div className="sm:hidden flex items-center w-full justify-end">
          {!mobileSearchOpen ? (
            ...
            <form className="flex items-center w-full transition-all duration-200" onSubmit={handleSubmit(onsubmit)}>
              <button
                type="button"
      ...
      {!mobileSearchOpen && (
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => navigate('/channeldashboard')} className="rounded-full flex flex-col items-center ml-2">
